...
<div dir="{{ $dir }}" class="font-custom max-w-4xl p-8 mx-auto my-8 bg-white rounded-lg "
    ...
    @php
        $invoiceCurrency = $sale->currency ?? \App\Models\Currency::getBaseCurrency();
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($sale->items as $item)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-gray-700 whitespace-nowrap text-{{ $textAlignStart }}">
                            {{ number_format($item->price, $decimalPlaces) }} {{ $currencySymbol }}</td>
                        <td class="px-4 py-3 text-gray-700 whitespace-nowrap text-{{ $textAlignStart }}">
                            {{ number_format($item->stock * $item->price, $decimalPlaces) }} {{ $currencySymbol }}</td>
                    </tr>
                @empty
                    <tr>
    ...
    <div class="flex justify-end mb-8 text-sm text-gray-700">
        <div class="w-full max-w-xs text-{{ $textAlignEnd }}">
            <div class="flex justify-between py-1">
                ...
                <span>{{ number_format($sale->sub_total, $decimalPlaces) }} {{ $currencySymbol }}</span>
            </div>
            <div class="flex justify-between py-1">
                <span>{{ __('Discount amount') }}</span>
                <span>-{{ number_format($sale->discount, $decimalPlaces) }} {{ $currencySymbol }}</span>
            </div>
            {{-- Optional Tax --}}
            {{-- <div class="flex justify-between py-1">
                <span>Tax (10%)</span>
                <span>{{ number_format(($sale->sub_total - $sale->discount_amount) * 0.10, 2) }} {{ __('IQD') }}</span>
            </div> --}}
            <div class="flex justify-between py-2 mt-2 font-semibold border-t-2 border-gray-200">
                <span>{{ __('Total amount') }}</span>
                <span>{{ number_format($sale->total_amount, $decimalPlaces) }}
                    {{ $currencySymbol }}</span>
            </div>
            {{-- Optional Paid/Due --}}
            @if (isset($sale->paid_amount))
                <div class="flex justify-between py-1 mt-2 text-green-600">
                    ...
                    <span>{{ number_format($sale->paid_amount, $decimalPlaces) }} {{ $currencySymbol }}</span>
                </div>
            @endif
            @if (isset($sale->amount_due))
                <div
                    ...
                    <span>{{ number_format($sale->amount_due, $decimalPlaces) }} {{ $currencySymbol }}</span>
                </div>
            @endif
        </div>
    </div>
