@php
    ...
    // Currency symbol (fallback to code, then IQD)
    $currencySymbol = $purchase->currency?->symbol ?? $purchase->currency?->code ?? __('IQD');
...
<div dir="{{ $dir }}" class="font-custom max-w-4xl p-8 mx-auto my-8 bg-white rounded-lg "
    ...
    <div class="flex items-center justify-between pb-6 mb-6 border-b border-gray-200">
        ...
        <div class="text-{{ $textAlignEnd }}">
            ...
            <img src="{{ Storage::url(setting('invoice.invoice_logo') ?? setting('general.logo')) }}" alt="Company Logo"
                class="h-16">
        </div>
    </div>
    ...
    <div class="flex justify-between mb-8">
        ...
        <div class="w-1/2 {{ $paddingStart }}-4 text-{{ $textAlignEnd }}">
            ...
            <p class="text-sm text-gray-600">{{ setting('invoice.your_company_name') }}</p>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($purchase->items as $item)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-gray-700 whitespace-nowrap text-{{ $textAlignStart }}">
                            {{ $item->stock . ' ' . $item->product?->unit?->abbreviation }}</td>
                        <td class="px-4 py-3 text-gray-700 whitespace-nowrap text-{{ $textAlignStart }}">
                            {{ number_format($item->cost, $currencyDecimals) }} {{ $currencySymbol }}</td>
                        <td class="px-4 py-3 text-gray-700 whitespace-nowrap text-{{ $textAlignStart }}">
                            {{ number_format($item->stock * $item->cost, $currencyDecimals) }} {{ $currencySymbol }}</td>
                    </tr>
                @empty
                    <tr>
    ...
    <div class="flex justify-end mb-8 text-sm text-gray-700">
        <div class="w-full max-w-xs text-{{ $textAlignEnd }}">
            <div class="flex justify-between py-1">
                ...
                <span>{{ number_format($purchase->sub_total, $currencyDecimals) }} {{ $currencySymbol }}</span>
            </div>
            <div class="flex justify-between py-1">
                <span>{{ __('Discount amount') }}</span>
                <span>-{{ number_format($purchase->discount_amount, $currencyDecimals) }} {{ $currencySymbol }}</span>
            </div>
            {{-- Optional Tax --}}
            {{-- <div class="flex justify-between py-1">
                <span>Tax (10%)</span>
                <span>{{ number_format(($purchase->sub_total - $purchase->discount_amount) * 0.10, $currencyDecimals) }} {{ $currencySymbol }}</span>
            </div> --}}
            <div class="flex justify-between py-2 mt-2 font-semibold border-t-2 border-gray-200">
                <span>{{ __('Total amount') }}</span>
                <span>{{ number_format($purchase->sub_total - $purchase->discount_amount, $currencyDecimals) }}
                    {{ $currencySymbol }}</span>
            </div>
            {{-- Optional Paid/Due --}}
            @if (isset($purchase->paid_amount))
                <div class="flex justify-between py-1 mt-2 text-green-600">
                    ...
                    <span>{{ number_format($purchase->paid_amount, $currencyDecimals) }} {{ $currencySymbol }}</span>
                </div>
            @endif
            @if (isset($purchase->amount_due))
                <div
                    ...
                    <span>{{ number_format($purchase->amount_due, $currencyDecimals) }} {{ $currencySymbol }}</span>
                </div>
            @endif
        </div>
    </div>
